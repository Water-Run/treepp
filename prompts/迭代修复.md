...
找出问题所在, 修正逻辑, 指引我替换代码; 最小的替换单元是一个完整的函数, 以避免我出错.  

---

你说得对，Rust 2024 edition 确实已经发布了。我更新分析并提供格式化的修改计划。
...
| 修改项目 | 问题 | 期望 | 涉及文件 | 如何实现 |
|---------|------|------|---------|---------|
| WinBanner 命令参数 | 使用了 `tree /f`，应该只用 `tree` 获取样板头 | 正确执行 `tree` 命令获取卷信息和序列号 | `render.rs` | 将 `"chcp 65001 >nul && tree /f"` 改为 `"tree"`，移除 chcp（让系统用原生编码） |
| WinBanner 编码处理 | 直接用 `String::from_utf8_lossy` 处理输出，但 Windows cmd 输出是 GBK/CP936 编码 | 正确解码系统编码的输出 | `render.rs` | 使用 `encoding_rs` 或 Windows API 检测系统编码并转换；或添加 `encoding_rs` 依赖处理 GBK |
| WinBanner 缓存 | 每次渲染都调用 `WinBanner::from_system()`，执行外部命令 | 程序启动时获取一次，后续复用 | `render.rs`, `main.rs` | 在 `main.rs` 启动时获取 `WinBanner`，通过参数传递给 render；或使用 `OnceLock` 全局缓存 |
| WinBanner 回退值语言 | 回退值是英文（`"Local Disk"`, `"No subfolders exist"`） | 回退值应匹配中文系统或保持空白让用户知道获取失败 | `render.rs` | 检测系统语言设置中文回退值，或直接使用空字符串/提示信息 |
...
| 修改项目 | 问题 | 期望 | 涉及文件 | 如何实现 |
|---------|------|------|---------|---------|
| gitignore 传递错误 | `parallel_scan_dir` 中重新加载当前目录的 gitignore 而非传递父级 | 正确传递父级 gitignore 规则链 | `scan.rs` | 修改 `parallel_scan_dir` 将 `effective_gitignore` 正确序列化传递，或改用 `Arc` 共享；参考原型 `match_rules` 的 `build_gitignore_chain` 实现 |
| gitignore 重复加载 | 每次进入子目录都调用 `load_gitignore`，无缓存 | 缓存已加载的 gitignore 规则 | `scan.rs` | 添加 `HashMap<PathBuf, Option<Gitignore>>` 缓存，参考原型 `match_rules` 的 `gitignore_cache` |
| 排序时机 | 扫描完成后全量递归排序 | 可在插入时保持有序，或接受当前实现（影响不大） | `scan.rs` | 维持现状或改用 `BTreeMap` 收集子节点 |

## 阶段三：输出体验改进

| 修改项目 | 问题 | 期望 | 涉及文件 | 如何实现 |
|---------|------|------|---------|---------|
| 日期时区 | `format_datetime` 使用 UTC 时间 | 显示本地时间，与 Windows tree 一致 | `render.rs` | 使用 `chrono` 库的 `Local` 时区，或通过 Windows API 获取本地时间；添加 `chrono = "0.4"` 依赖 |
...
| 修改项目 | 问题 | 期望 | 涉及文件 | 如何实现 |
|---------|------|------|---------|---------|
| `--quote` / `-q` / `/Q` | 未实现 | 用双引号包裹文件名 | `cli.rs`, `config.rs`, `render.rs` | 添加 `ArgDef`；`RenderOptions` 加 `quote_names: bool`；`format_entry_name` 中包裹引号 |
| `--dirs-first` / `-D` / `/DF` | 未实现 | 目录优先显示（当前已实现但未暴露参数） | `cli.rs`, `config.rs` | 添加 `ArgDef`；排序逻辑已有目录优先，只需添加开关控制 |

## 阶段五：代码质量

| 修改项目 | 问题 | 期望 | 涉及文件 | 如何实现 |
|---------|------|------|---------|---------|
| 根路径验证时机 | `Config::validate` 未检查路径存在性，在 `scan` 时才检查 | 尽早失败，在配置验证阶段检查 | `config.rs` | 在 `validate()` 中添加 `if !self.root_path.exists()` 检查 |
| dunce 未使用 | 引入了 `dunce` 依赖但未使用，Windows 路径可能有 `\\?\` 前缀问题 | 使用 dunce 规范化路径 | `config.rs` 或 `scan.rs` | 在获取根路径规范路径时调用 `dunce::canonicalize()` |
| 路径位置验证 | 允许路径出现在选项之后 | 路径必须在选项之前（与原型一致） | `cli.rs` | 添加 `has_seen_option` 标记，参考原型 `cli_compat` 的 `PathAfterOptions` 错误 |
| 错误类型重复 | `CliError::InvalidValue` 和 `ConfigError::InvalidValue` 职责重叠 | 明确边界：CLI 层处理格式错误，Config 层处理语义错误 | `error.rs`, `config.rs` | 重新审视错误划分，或合并为统一错误类型 |

## 阶段六：测试与验证

| 修改项目 | 问题 | 期望 | 涉及文件 | 如何实现 |
|---------|------|------|---------|---------|
| 一致性验证 | 原型有 `verify_consistency` 验证 walk/parallel 结果一致，正式版未整合 | 提供验证命令或测试 | `scan.rs`, 新增 `tests/` | 整合原型 `scan_engine` 的 `ConsistencyReport` 和 `verify_consistency`；添加集成测试 |
| 性能基准 | 原型有 `WARMUP_RUNS`、`BENCHMARK_RUNS` 框架，正式版未整合 | 提供 benchmark 命令 | 新增 `benches/` | 使用 `criterion` 或整合原型的基准测试逻辑 |
| 原生 tree 对比 | 原型有 `scan_native_tree` 对比功能 | 验证输出与原生 tree 一致 | `tests/` | 整合原型的 `scan_native_tree`，添加对比测试 |
...
# Cargo.toml 需要添加的依赖
[dependencies]
encoding_rs = "0.8"      # GBK 编码处理（WinBanner）
chrono = "0.4"           # 本地时区日期格式化

[dev-dependencies]
criterion = "0.5"        # 性能基准测试（可选）
```
...
1. **阶段一（立即）**：修复 WinBanner，这是最明显的用户可见问题
2. **阶段二（短期）**：性能优化，尤其是 gitignore 相关
3. **阶段五（短期）**：根路径验证、dunce 使用等代码质量问题
4. **阶段四（中期）**：补全缺失参数
5. **阶段三（中期）**：日期时区；实时输出可延后
6. **阶段六（持续）**：测试覆盖